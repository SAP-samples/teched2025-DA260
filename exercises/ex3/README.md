...
6.  Click on **‘Select Source Container’** button and it launches the Select Container popup. Then click on the **DA260**. After selecting DA260, please click the **Select** button.
...
-   BusinessPartners <br>
   ...
   <br>
Please keep in kind that you need to repeat this step for each of the csv files that you want to add, and then click the select button.
...
10. Click on *Add Selection *that starts the fetching of source objects details from the source.
    ...
    After you have added the 'Product.csv' file, you need to select the **'+'** button to add the other two source objects (BusinessPartners.csv & SalesOrders.csv), as depicted in the image below:

    ![ex_03_30](images/ex_03_30.png)

    You need to repeat steps 9-10 for the other 2 files that are required for this exercise.

11. The three selected CSV Files are now added to the Replication Flow:

    ![ex_03_12](images/ex_03_12.png)

12. You see some red notifications that are being shown on the left of each of the Source Objects, you need to select each of the datasets, than on the right side, the Replication Flow Properties window is opened, and you need to scroll down and select **Configure Schema**. A new pop-up window will open, and on the left side you will have different properties for the csv files. For the sake of this exercise please do not change the properties, keep them as they are for each of the source objects. First, select the primary key for the dataset, select *Apply Changes* button and after applying the changes you need again to  select the check box for the **Primary Key** of the source object and click the *Save* button.
